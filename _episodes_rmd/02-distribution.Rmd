...
Remember that a boxplot visualises the distribution of a variable. The values
of that variable is all we need:
...
some Chinstraps and Adelies are as heavy as some Gentoos. 

This overlap does not mean that we can conclude that Gentoos are not larger than 
Adelie penguins. Or that Adelie and Gentoo penquins are really the same. Or for
that matter that penguins exist on a spectrum between Gentoos and Adelies.
...
As mentioned boxplots might hide interesting features. The two plots below show first two 
